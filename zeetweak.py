...
'''
ZeeTweak 4.0 - Transfer and process data for the Zeeweii DSO3D12 oscilloscope.
...
BASE_HW_ADDR = 0x2001dce0 # Global settings/status data memory address, firmware v3.0.7
VDIV_BASE_X1 = {13:10.0, 12:5.0, 11:2.0, 10:1.0, 9:0.5, 8:0.2, 7:0.1, 6:0.05, 5:0.02, 4:0.01, 3:0.005, 2:0.002, 1:0.001}
TIMEBASE_LABELS = {
...
SCOPE_DATA = {
    ...
    1201: ("System Status?", 1, False),             # 4=Ready?
    ...
    1264: ("Trigger Level Raw ADC?", 2, False),     # 0-255
    1266: ("CH1 V/Div Index", 1, False),
    1268: ("CH1 Volt Scale (10uV)", 4, False),
    1272: ("CH1 Vert Offset (pixels)", 4, True),
    1276: ("CH1 Vert Offset (10uV)", 8, True),
    1284: ("CH1 Trigger Level Target (10uV)", 8, True),
    1292: ("CH1 Trigger Level (10uV)", 8, True),
    ...
    1316: ("CH1 Vert Offset Buffer (10uV)", 8, True),
    1324: ("CH1 Vert Scroll Offset", 4, False),
    1328: ("CH2 V/Div Index", 1, False),
    1332: ("CH2 Volt Scale (10uV)", 4, False),
    1336: ("CH2 Vert Offset (pixels)", 4, True),
    1340: ("CH2 Vert Offset (10uV)", 8, True),
    1348: ("CH2 Trigger Level Target (10uV)", 8, True),
    1356: ("CH2 Trigger Level (10uV)", 8, True),
    ...
    1464: ("CH1 Color Index", 2, False),
    1466: ("0x2001ddea", 2, False),
    ...
    1487: ("Fine-Course Movement?", 1, False),
    ...
    1509: ("0x2001de15", 1, False),
    1512: ("Horiz Window Pos? (ps)", 8, True),
...
def format_voltage_val(volts):
    val = abs(volts)
    if val == 0: return "0.00V"
    if val < 0.001: return f"{volts * 1_000_000:.0f}uV"
    if val < 1.0: return f"{volts * 1000:.0f}mV"
    if val < 10.0: return f"{volts:.2f}V"
    if val < 100.0: return f"{volts:.1f}V"
    return f"{volts:.0f}V"
...
def format_delay_val(seconds):
    val = abs(seconds)
    if val == 0: return "0.00s"
    if val < 1e-6: return f"{seconds * 1e9:.1f}ns"
    if val < 1e-3: return f"{seconds * 1e6:.3f}us"
    if val < 1.0: return f"{seconds * 1e3:.3f}ms"
...
def debug_diff(current_data):
    ...
    changes = []
    covered_indices = set()
    for abs_offset, (label, size, is_signed) in SCOPE_DATA.items():
...
def parse_debug(buf, f):
    ...
    while curr < end_offset:
        hw_addr = BASE_HW_ADDR + (curr - start_offset)
        if curr in SCOPE_DATA:
            label, size, is_signed = SCOPE_DATA[curr]
...
def parse_all_measurements(data, offset):
    chunk = data[offset : offset+100]
    if len(chunk) < 100: return []
    uv = struct.unpack('<25I', chunk)
    measurements = []
    freq_uhz = (uv[23] << 32) | uv[22]
    measurements.append(("Freq", format_freq(freq_uhz / 1000000.0)))
    measurements.append(("T", format_time_val(uv[20] / 1e12)))
    measurements.append(("+T", format_time_val(uv[16] / 1e12)))
    measurements.append(("-T", format_time_val(uv[18] / 1e12)))
    packed = uv[24]
    measurements.append(("+Duty", f"{(packed & 0xFFFF) / 10.0:.1f}%"))
    measurements.append(("-Duty", f"{(packed >> 16) / 10.0:.1f}%"))
    v_map = [
        ("PkPk", 0), ("Amp", 2), ("Max", 8), ("Min", 10),
        ("Top", 4), ("Base", 6), ("Avg", 12), ("RMS", 14)
    ]
    for label, idx in v_map:
        raw = uv[idx]
        if raw > 0x7FFFFFFF: raw -= 0x100000000
        measurements.append((label, format_voltage_val(raw / 100000.0))) 
    return measurements


def parse_active_measurements_list(data, offset, flag_offset, forced_bits=None):
    chunk = data[offset : offset+100]
    if len(chunk) < 100: return []
    uv = struct.unpack('<25I', chunk)
    ...
    for bit in enabled_bits:
        if bit in MEASUREMENT_FLAGS:
            ...
            if mtype == "F": 
                freq_uhz = (uv[23] << 32) | uv[22]
                active_meas.append((label, format_freq(freq_uhz / 1e6)))
            elif mtype == "D+" or mtype == "D-":
                packed = uv[24]
                val = (packed & 0xFFFF) if mtype == "D+" else (packed >> 16)
                active_meas.append((label, f"{val/10.0:.1f}%"))
            else: 
                raw = uv[val_idx]
                if mtype == "V":
                    if raw > 0x7FFFFFFF: raw -= 0x100000000
                    active_meas.append((label, format_voltage_val(raw / 100000.0)))
...
def process_capture(buf, filename, args):
    state = {}
    for offset, (label, size,is_signed) in SCOPE_DATA.items(): state[label] = get_val(buf, offset, size, signed=is_signed)
    title_text = None
    if args.title:
    ...
    with open(f"{filename}.txt", 'w') as f:
        ...
        # CH1
        c1_vdiv = format_voltage_val(VDIV_BASE_X1.get(state["CH1 V/Div Index"], 0) * (10**state["CH1 Probe Mode"]))
        ...
        # CH2
        c2_vdiv = format_voltage_val(VDIV_BASE_X1.get(state["CH2 V/Div Index"], 0) * (10**state["CH2 Probe Mode"]))
        ...
        if state["CH2 Enabled"]: f.write(f"Channel 2:      ")
        else: f.write(f"Channel 2[off]: ")
        f.write(f"{c2_vdiv}/div | {c2_cpl} | {c2_prb}{c2_bw_str}\n")
        ...
        trig_src = "CH1" if trig_src_idx == 0 else "CH2"
        trig_mode = "Auto" if state["Trigger Mode"] == 0 else "Normal"
        trig_edge = "Rise" if state["Trigger Edge"] == 0 else "Fall"
        if trig_src_idx == 0: trig_val_10uv = state.get("CH1 Trigger Level (10uV)", 0)
        else: trig_val_10uv = state.get("CH2 Trigger Level (10uV)", 0)
        trig_volts = trig_val_10uv * 0.00001
        if abs(trig_volts) < 1.0: trig_lvl_str = f"{trig_volts * 1000:.0f}mV"
        else: trig_lvl_str = f"{trig_volts:.2f}V"
        f.write(f"Trigger:        {trig_src} | {trig_mode} | {trig_edge} | {trig_lvl_str}\n") 
        delay_ps = state.get("Horiz Trigger Delay (ps)", 0)
        delay_str = format_delay_val(delay_ps / 1e12)
        f.write(f"Horiz Delay:    {delay_str}\n")

        # System
        sr_val = state.get("Sample Rate (Hz)", 0)
        sr_str = format_sample_rate(sr_val)
        f.write(f"Sample Rate:    {sr_str}\n")
        f.write(f"Signal Gen:     ")
        if state["SigGen Enable"]:
            wave = SIGGEN_PATTERNS.get(state["SigGen Waveform"], "Unknown")
        ...
        # Measurements
        f.write("\nCH1 Measurements:\n")
        meas_c1 = parse_all_measurements(buf, 1540)
        for label, val in meas_c1: f.write(f"  {label:<6}: {val}\n")
        
        f.write("\nCH2 Measurements:")
        if state["CH2 Enabled"]:
            f.write("\n")
            meas_c2 = parse_all_measurements(buf, 1640)
    ...
    if ch2_color is None:
        color_val = state["Settings-CH2 Color"]
    ...
    fig_height_in = grid_in_h + bottom_panel_h_in + title_height_in
    ...
    # Grid
    ax_grid = fig.add_axes([0.02, bottom_panel_h_norm, grid_w_norm - 0.03, grid_h_norm - 0.03])
    ...
    meas_c1_list = parse_active_measurements_list(buf, 1540, 1308, forced_bits)
    ...
    meas_c2_list = parse_active_measurements_list(buf, 1640, 1372, forced_bits)
    ...
    if trig_src == 0:
        trig_val = state.get("CH1 Trigger Level (10uV)", 0)
        scale = state.get("CH1 Volt Scale (10uV)", 100000)
    else:
        trig_val = state.get("CH2 Trigger Level (10uV)", 0)
        scale = state.get("CH2 Volt Scale (10uV)", 100000)
    if scale == 0: scale = 1
    ...
    # Zero voltage arrow
    c1_offset_10uv = state.get("CH1 Vert Offset (10uV)", 0) 
    c1_scale_10uv  = state.get("CH1 Volt Scale (10uV)", 100000) 
    if c1_scale_10uv == 0: c1_scale_10uv = 1
    c1_offset_in_divs = c1_offset_10uv / c1_scale_10uv
    c1_zero_y = 128 + int(c1_offset_in_divs * 25.0)
    ...
    if state["CH2 Enabled"]:
        c2_offset_10uv = state.get("CH2 Vert Offset (10uV)", 0)
        c2_scale_10uv  = state.get("CH2 Volt Scale (10uV)", 100000)
        if c2_scale_10uv == 0: c2_scale_10uv = 1
        c2_offset_in_divs = c2_offset_10uv / c2_scale_10uv
        c2_zero_y = 128 + int(c2_offset_in_divs * 25.0)
    ...
    # Cursors
    if state["Cursors X Enable"]:
        cx1 = 150 + state["Cursor X1 Pos"]
        cx2 = 150 + state["Cursor X2 Pos"]
        ax_grid.axvline(cx1, color='green', linestyle=':')
        ax_grid.axvline(cx2, color='green', linestyle=':')
        
    if state["Cursors Y Enable"]:
        cy1 = 128 + state["Cursor Y1 Pos"]
        cy2 = 128 + state["Cursor Y2 Pos"]
        ax_grid.axhline(cy1, color='green', linestyle=':')
        ax_grid.axhline(cy2, color='green', linestyle=':')

    ax_grid.axis('off')

    # Bottom panel
    ax_bottom = fig.add_axes([0.02, 0.01, grid_w_real, bottom_panel_h_norm])
...
def main():
    ...
    try:
        while True:
            ...
            while ser is None:
                try:
                    ser = serial.Serial(args.source, 115200, timeout=10)
